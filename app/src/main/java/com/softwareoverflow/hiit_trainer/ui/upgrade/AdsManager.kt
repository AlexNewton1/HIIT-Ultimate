...
class AdsManager(private val context: Context, private val bannerAd: AdView) {
    ...
    companion object {
        ...
        fun showAdAfterWorkout(onAdClosedCallback: () -> Unit) {
            // Only show the advert if it's loaded, the user is a free user, and they won't be asked for feedback this time.
            if (!hasUserUpgraded && workoutEndInterstitial.isLoaded() && !InAppReviewManager.willAskForReview) {
