 ...
 *
 * - Security: Imagine a situation where four users decide to share one premium car purchase amongst themselves. So one user would buy the
 ...
 * to give it a different name. Also recall that [BillingRepository] might handle three different
 * data sources (Play, local cache & secure backend server).
 * So letting each source call its data by a slightly different name might make
